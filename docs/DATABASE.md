...
This guide explains how to set up and use the PostgreSQL database with Drizzle ORM in Fainansu.
...
```env
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/fainansu
```
...
#### Backup
```bash
docker exec fainansu-db pg_dump -U postgres fainansu > backup.sql
```

#### Restore
```bash
docker exec -i fainansu-db psql -U postgres fainansu < backup.sql
```

#### Automated Backups (Cron)
```bash
# Add to crontab
0 2 * * * docker exec fainansu-db pg_dump -U postgres fainansu | gzip > /backups/fainansu-$(date +\%Y\%m\%d).sql.gz
```
...
docker exec -it fainansu-db psql -U postgres fainansu
\dt  # List tables
